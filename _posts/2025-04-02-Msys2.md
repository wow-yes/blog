...
因此在MSYS2，使用 pip 直接安装某个包，pip 会把源代码下载下来，然后用VS编译，但似乎其不能正常识别 MSYS2 的环境变量，因此错误频出。

我只是使用Python 作为分析数据的工具，因此自行编译Python对我来说得不偿失。因此作罢。还是安装官方出品的Windows版，来作为Vscode的后端比较好。
