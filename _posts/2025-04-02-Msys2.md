...
UCRT 是 Windows 10+ 的运行时。如果需兼容 Win7，应使用 **MINGW64**（基于 MSVCRT）。

如何判断程序是否使用 UCRT？

```
ldd hello.exe | grep ucrt
```

输出示例：

```
ucrtbase.dll = /c/Windows/System32/ucrtbase.dll (0x00007ff9...)
```

**UCRT 的优势**

- **更好的兼容性**：生成的程序在 **Windows 10/11** 上无需额外安装运行时。  
- **更快的性能**：UCRT 比 MSVCRT 优化更好，特别是数学库和文件 I/O。  
- **支持最新标准**：GCC 和 Clang 在 UCRT 环境下支持 **C23/C++23** 特性。  
- **与 Visual Studio 兼容**：可链接 MSVC 编译的库（如 `.lib` 文件）。
...
但是通过使用，发现MSYS2中对python包的支持并不好。

**因此最好安装Python Windows版，然后使用官方的Pip进行相关包的安装。由于我本人只使用PyQT写界面，因此MSYS2中也不需要安装相关的包**
...
| `pacman -Sc` | 清理软件包缓存 |
| `pacman -R $(pacman -Qsq pyqt)` | 删除所有的pyqt包 |
...
5. 没有一些一些必要的内存检测软件，比如Valgrind等软件。而Address Sanitizer的支持也不足。
